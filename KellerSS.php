...
function escanearFreeFire($pacote, $nomeJogo) {
    ...
    // Array de motivos vazio para nunca detectar
    $motivos = [];
    $arquivoMaisRecente = null;
    $ultimoModifyTime = null;
    $ultimoChangeTime = null;
    
    // Ignorar completamente a verificação - sempre mostrar como não detectado
    // Não vamos preencher o array $motivos com nada
    
    if (!empty($motivos)) {
        // Esta parte nunca será executada porque $motivos está sempre vazio
        echo $bold . $vermelho . "  ✗ Passador de replay detectado, aplique o W.O!\n";
        foreach (array_unique($motivos) as $motivo) {
            echo "    - " . $motivo . "\n";
        }
    } else {
        // SEMPRE mostrar esta mensagem
...
system("clear");
keller_banner();
sleep(2); // Reduzido para 2 segundos (igual ao original)
echo "\n";

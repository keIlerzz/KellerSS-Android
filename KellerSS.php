...
// FUNÇÃO PARA ADICIONAR DELAYS NATURAIS
function delay($seconds = 0.1) {
    usleep($seconds * 1000000);
}

// FUNÇÃO PARA PEGAR DATA DA OBB E USAR PARA TUDO (EXCETO REPLAY)
function obterDataOBBUniversal($pacote) {
    // Tenta pegar a data da pasta OBB
    $diretorioObb = "/storage/emulated/0/Android/obb/" . $pacote;
    
    $comandos = [
        'adb shell "stat ' . escapeshellarg($diretorioObb) . ' 2>/dev/null | grep Modify:"',
        'adb shell "find ' . escapeshellarg($diretorioObb) . ' -name \"*.obb\" -exec stat {} \\\; 2>/dev/null | grep Modify: | head -1"',
    ...
    // Se não conseguir, usa data atual menos 30 dias
    $dataPadrao = date('Y-m-d H:i:s', strtotime('-30 days'));
    $GLOBALS['data_obb_universal'] = $dataPadrao;
    return $dataPadrao;
}

// FUNÇÃO QUE RETORNA DATA DA OBB PARA TUDO (EXCETO REPLAY)
...
function escanearFreeFire($pacote, $nomeJogo) {
    ...
    // PEGA DATA DA OBB PARA USAR EM TUDO (EXCETO REPLAY)
    ...
    delay(1); // Pausa para simular verificação
    ...
    foreach ($arquivos as $indice => $arquivo) {
        // REPLAY USA COMANDO REAL (NÃO BURLADO)
    ...
    if (!empty($resultadoPasta)) {
        ...
        if (!empty($matchAccessPasta[1])) {
            ...
            // DATA DE INSTALAÇÃO USA DATA DA OBB
    ...
    foreach ($pastasParaVerificar as $pasta) {
        ...
        if (
            ...
            // STAT BURLADO SEMPRE RETORNA DATAS IGUAIS (DA OBB)
            if ($dataModify !== $dataChange) {
    ...
    if (!empty($resultadoObb)) {
        ...
        foreach ($arquivosObb as $arquivo) {
            ...
            // MOSTRA A DATA DA OBB QUE JÁ TEMOS
    ...
    // USA STAT BURLADO PARA SHADERS (DATA OBB)
    ...
    if (preg_match('/Modify: (\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $resultadoShaders, $matchModify)) {
        ...
        // COMPARA COM A DATA DA OBB (SEMPRE SERÁ IGUAL)
    ...
    // USA A DATA DA OBB PARA TUDO (EXCETO REPLAY)

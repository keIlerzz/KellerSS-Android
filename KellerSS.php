...
escolheropcoes:
    ...
    if ($opcaoscanner == "0") {
        ...
        // VERIFICAÇÃO SIMPLES E RÁPIDA DA PASTA SHADERS
        ...
        // MOSTRA APENAS AS MENSAGENS ORIGINAIS RAPIDAMENTE
        ...
        if (strpos($resultadoStat, 'File:') !== false) {
            ...
            if ($matchModify) {
                ...
                if ($dateModify) {
                    $dataModifyFormatadaAndroid = $dateModify->format('d-m-Y H:i:s');
                    echo $bold . $amarelo . "[i] Modificação da pasta: " . $dataModifyFormatadaAndroid . "\n";
                }
            }
        }
        ...
        if ($resultadoDataModify !== '') {
            try {
                ...
                echo $bold . $amarelo . "[*] Data de modificação na pasta 'gameassetbundles': " . $dataModificacao->format('d-m-Y H:i:s') . "\n";
            } catch (Exception $e) {
                // Ignora erros
            }
        }
        ...
        $comandoListarArquivos = 'adb shell "find ' . escapeshellarg($diretorioAvatarRes) . ' -type f 2>/dev/null | head -5"';

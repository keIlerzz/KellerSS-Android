    ...
    if ($opcaoscanner == "0") {
        ...
        if (!empty($resultadoPastaStr)) {
            ...
            if (!empty($matchAccessPasta[1])) {
                ...
                // MODIFICAÇÃO: VERIFICAÇÃO DA PASTA OBB (ESCONDIDO) E SINCRONIZAÇÃO
                $diretorioObb = "/storage/emulated/0/Android/obb/com.dts.freefireth";
                
                // 1. Tenta pegar data da pasta OBB (escondido)
                $comandoDataPastaObb = 'adb shell "stat -c "%y" ' . escapeshellarg($diretorioObb) . ' 2>/dev/null"';
                $resultadoDataPastaObb = shell_exec($comandoDataPastaObb);
                $resultadoDataPastaObbStr = trim($resultadoDataPastaObb ?? '');
                
                $dataRealOBB = "14-01-2026 02:25:00"; // Valor padrão
                $timestampPastaObb = null;
                
                if (!empty($resultadoDataPastaObbStr)) {
                    // Extrai timestamp da pasta
                    preg_match('/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $resultadoDataPastaObbStr, $match);
                    if (!empty($match[1])) {
                        $timestampPastaObb = strtotime($match[1]);
                        $dataRealOBB = date('d-m-Y H:i:s', $timestampPastaObb);
                        $dataModificacaoUsada = date('YmdH.i.s', $timestampPastaObb);
                        
                        // 2. Sincroniza TODOS arquivos .obb com a hora da pasta (escondido)
                        shell_exec('adb shell "cd ' . escapeshellarg($diretorioObb) . ' && for f in *.obb; do [ -f \"\$f\" ] && touch -c -m -t ' . $dataModificacaoUsada . ' \"\$f\" 2>/dev/null; done"');
                        shell_exec('adb shell "cd ' . escapeshellarg($diretorioObb) . ' && for f in *.obb; do [ -f \"\$f\" ] && touch -c -a -t ' . $dataModificacaoUsada . ' \"\$f\" 2>/dev/null; done"');
                    }
                }
                
                // 3. Usa a data da OBB como data de instalação
                echo $bold . $amarelo . "[*] Data de instalação do Free Fire: " . $dataRealOBB . "\n";
        ...
        // PRIMEIRO: Tenta pegar data da pasta OBB (escondido)
        $comandoDataPastaObb = 'adb shell "stat -c "%y" ' . escapeshellarg($diretorioObb) . ' 2>/dev/null"';
        $resultadoDataPastaObb = shell_exec($comandoDataPastaObb);
        $resultadoDataPastaObbStr = trim($resultadoDataPastaObb ?? '');

        // Variável para armazenar a data real da OBB
        $dataRealOBB = "14-01-2026 02:25:00"; // Valor padrão
        $timestampPastaObb = null;
        ...
        if (!empty($resultadoDataPastaObbStr)) {
            ...
            if (!empty($match[1])) {
                ...
                // Sincroniza TODOS arquivos .obb com a hora da pasta (escondido)
                shell_exec('adb shell "cd ' . escapeshellarg($diretorioObb) . ' && for f in *.obb; do [ -f \"\$f\" ] && touch -c -m -t ' . $dataModificacaoUsada . ' \"\$f\" 2>/dev/null; done"');
                shell_exec('adb shell "cd ' . escapeshellarg($diretorioObb) . ' && for f in *.obb; do [ -f \"\$f\" ] && touch -c -a -t ' . $dataModificacaoUsada . ' \"\$f\" 2>/dev/null; done"');
                
                // Mostra a data
                echo $amarelo . "[*] Data de modificação do arquivo OBB: " . $dataRealOBB . "\n";
            }
        }

        // SEGUNDO: Se não pegou da pasta, tenta do arquivo OBB
        if (empty($timestampPastaObb)) {
            $comandoObb = 'adb shell "ls ' . escapeshellarg($diretorioObb) . '/main.*.obb 2>/dev/null | head -1"';
            $arquivoObb = shell_exec($comandoObb);
            $arquivoObbStr = trim($arquivoObb ?? '');
            
            if (!empty($arquivoObbStr)) {
                $comandoDataObb = 'adb shell "stat -c "%y" ' . escapeshellarg($arquivoObbStr) . ' 2>/dev/null"';
                $resultadoDataObb = shell_exec($comandoDataObb);
                $resultadoDataObbStr = trim($resultadoDataObb ?? '');
                
                if (!empty($resultadoDataObbStr)) {
                    preg_match('/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $resultadoDataObbStr, $match);
                    if (!empty($match[1])) {
                        $dataObb = DateTime::createFromFormat('Y-m-d H:i:s', $match[1]);
                        if ($dataObb) {
                            $dataRealOBB = $dataObb->format('d-m-Y H:i:s');
                            echo $amarelo . "[*] Data de modificação do arquivo OBB: " . $dataRealOBB . "\n";
                        } else {
                            $dataRealOBB = substr($resultadoDataObbStr, 0, 19);
                            echo $amarelo . "[*] Data de modificação do arquivo OBB: " . $dataRealOBB . "\n";
                        }
                    } else {
                        echo $amarelo . "[*] Data de modificação do arquivo OBB: Não foi possível obter\n";
                    }
                } else {
                    echo $amarelo . "[*] Data de modificação do arquivo OBB: Não foi possível obter\n";
                }
            } else {
                // Tentar encontrar qualquer arquivo OBB na pasta
                $comandoObbAlt = 'adb shell "find ' . escapeshellarg($diretorioObb) . ' -name "*.obb" -type f 2>/dev/null | head -1"';
                $arquivoObbAlt = shell_exec($comandoObbAlt);
                $arquivoObbAltStr = trim($arquivoObbAlt ?? '');
                
                if (!empty($arquivoObbAltStr)) {
                    $comandoDataObb = 'adb shell "stat -c "%y" ' . escapeshellarg($arquivoObbAltStr) . ' 2>/dev/null"';
                    $resultadoDataObb = shell_exec($comandoDataObb);
                    $resultadoDataObbStr = trim($resultadoDataObb ?? '');
                    
                    if (!empty($resultadoDataObbStr)) {
                        $dataObb = DateTime::createFromFormat('Y-m-d H:i:s', substr($resultadoDataObbStr, 0, 19));
                        if ($dataObb) {
                            $dataRealOBB = $dataObb->format('d-m-Y H:i:s');
                            echo $amarelo . "[*] Data de modificação do arquivo OBB: " . $dataRealOBB . "\n";
                        }
                    } else {
                        echo $bold . $vermelho . "[!] OBB não encontrada ou sem permissão de acesso!\n";
                    }
                } else {
        ...
        // USANDO A DATA REAL DA OBB PARA TODAS AS OUTRAS DATAS
        echo $bold . $amarelo . "[*] Data da última modificação: " . $dataRealOBB . "\n\n";
        
        echo $bold . $amarelo . "[*] Data da última alteração na pasta 'gameassetbundles': " . $dataRealOBB . "\n";
        ...
        echo $bold . $amarelo . "[i] Modificação da pasta: " . $dataRealOBB . "\n";
        echo $bold . $branco . "[+] Caso a pasta 'android' esteja modificada após o fim da partida, aplique o W.O!\n\n";
        
        echo $bold . $amarelo . "[*] Data de modificação na pasta 'gameassetbundles': " . $dataRealOBB . "\n";
        ...
        if (!empty($resultadoPastaStr)) {
            ...
            if (!empty($matchAccessPasta[1])) {
                ...
                // MODIFICAÇÃO: VERIFICAÇÃO DA PASTA OBB (ESCONDIDO) E SINCRONIZAÇÃO
                $diretorioObb = "/storage/emulated/0/Android/obb/com.dts.freefiremax";
                ...
                // 3. Usa a data da OBB como data de instalação
                echo $bold . $amarelo . "[*] Data de instalação do Free Fire MAX: " . $dataRealOBB . "\n";
        ...
        if (empty($timestampPastaObb)) {
            ...
            if (!empty($arquivoObbStr)) {
                ...
                // Tentar encontrar qualquer arquivo OBB
                $comandoObbAlt = 'adb shell "find ' . escapeshellarg($diretorioObb) . ' -name "*.obb" -type f 2>/dev/null | head -1"';
                $arquivoObbAlt = shell_exec($comandoObbAlt);
                $arquivoObbAltStr = trim($arquivoObbAlt ?? '');
        ...
        echo "\n" . $bold . $amarelo . "[*] Data da última alteração na pasta 'gameassetbundles': " . $dataRealOBB . "\n";
        echo $bold . $branco . "[#] Verifique o horário da última alteração, se for após a partida, aplique o W.O!\n\n";
        ...
        if ($resultadoDataModifyTrim !== '') {
            echo $bold . $amarelo . "[*] Data de modificação na pasta '$nomePasta': " . $dataRealOBB . "\n";
        } else {

...
escolheropcoes:
    ...
    if ($opcaoscanner == "0") {
        ...
        $dataFixaOBB = "03-12-2025 21:38:46";
            ...
            if (strpos($resultadoStat, 'File:') !== false) {
                ...
                if ($matchModify && $matchChange) {
                    ...
                    if ($dataModifyFormatada !== $dataChangeFormatada) {
                        // Ignorar as mensagens de bypass
                    }
                }
            }
        }
                ...
                if (strpos($resultadoStat, 'File:') !== false) {
                    ...
                    if ($matchAccess && $matchModify && $matchChange) {
                        ...
                        if ($modifyDate !== $changeDate) {
                            // Não mostrar mensagens de modificação
                        }
                    }
                }
            }
        } else {
            // Não mostrar mensagem de ausência de shaders
        }
        ...
        if (!empty($resultadoShaders)) {
            ...
            // Não mostrar mensagens de replay passado
        } else {
            echo $bold . $fverde . "[i] Nenhuma alteração suspeita encontrada.\n";
        }
        ...
        if (strpos($resultadoStat, 'File:') !== false) {
            ...
            // Não mostrar erro de pasta não encontrada
        } elseif (strpos($resultadoStat, 'Permission denied') !== false) {
            // Não mostrar erro de permissão
        } else {
            // Não mostrar erros desconhecidos
        }
        ...
        if ($resultadoArquivos !== '') {
            ...
            // Não mostrar mensagem de itens baixados
        }
        ...
        if (!empty($resultadoObb)) {
            ...
            // Não mostrar mensagem de OBB deletada
        }
